;set long word to for loop
;set constants to double prec.
...
w = where(a LT 15.)
R = 0.0166667d0/tan((a + 7.31d0/(a+4.4d0))*d2r)

;R = 1.02/tan((a + 10.3/(a+5.11))*d2r)/60. 
; this formula goes the other direction!

if w[0] ne -1 then R[w] = 3.569d0*(0.1594d0 + 0.0196d0*a[w] + $
      0.00002d0*a[w]^2.d0)/(1.d0+0.505d0*a[w]+0.0845d0*a[w]^2.d0)
tpcor = P/1010.d0 * 283.d0/T
R = tpcor * R
...
function co_refract_mod, a, altitude=altitude, pressure=pressure,  $
...
alpha = 0.0065d0 ; temp lapse rate [deg C per meter]

if n_elements(altitude) eq 0 then altitude = 0.d0
if n_elements(temperature) eq 0 then begin
        if altitude GT 11000 then temperature = 211.5d0 $
                             else temperature = 283.0d0 - alpha*altitude
endif
; estimate Pressure based on altitude, using U.S. Standard Atmosphere formula.
if n_elements(pressure) eq 0 then $ 
              pressure = 1010.d0*(1.d0-6.5d0/288000.d0*altitude)^5.255d0
if n_elements(epsilon) eq 0 then  $
     epsilon = 0.25d0 ; accuracy of iteration for observed=1 case, in arcseconds
...
; expand those to be arrays of the same size.
	P = pressure + a*0.d0 & T = temperature + a*0.d0
        ...
        for i=0L,na-1 do begin
            ...
            cur = a[i] + dr ; guess of observed location
                ...
                repeat begin
                  ...
                  cur= a[i] + dr
                endrep until abs(last-cur)*3600.d0 LT epsilon
                aout[i] = cur
        endfor
endelse

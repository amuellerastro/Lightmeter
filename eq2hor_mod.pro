;uses co_refract_mod
;some stuff comment out
;set double prec.
;added fully corrected RA,DEC to output -- removed
...
pro eq2hor_mod, ra, dec, jd, alt, az, ha, lat=lat, lon=lon, WS=WS, obsname=obsname,$
...
J_now = (JD - 2451545.d0)/365.25d0 + 2000.0d0 ; compute current equinox
if precess_ then begin
        if keyword_set(B1950) then begin
                for i=0,n_elements(jd)-1 do begin
                        ra_i = ra_[i] & dec_i = dec_[i]
                        precess, ra_i, dec_i, 1950.0d0, J_now[i], /FK4
                        ra_[i] = ra_i & dec_[i] = dec_i
                endfor
        endif else begin
                for i=0,n_elements(jd)-1 do begin
                        ra_i = ra_[i] & dec_i = dec_[i]
                        precess, ra_i, dec_i, 2000.0d0, J_now[i]
                        ra_[i] = ra_i & dec_[i] = dec_i
                endfor
        endelse
endif
...
ra_ = ra_ + (dra1*nutate_ + dra2*aberration_)/3600.d0
dec_ = dec_ + (ddec1*nutate_ + ddec2*aberration_)/3600.d0
...
; calculate local APPARENT sidereal time
LAST = lmst + d_psi *cos(eps)/3600.d0 ; add correction in degrees
if v then print, 'LMST = ', adstring(lmst/15.d0)
if v then print, 'LAST = ', adstring(last/15.d0)

;******************************************************************************
; Find hour angle (in DEGREES)
ha = last - ra_
w = where(ha LT 0)
if w[0] ne -1 then ha[w] = ha[w] + 360.d0
ha = ha mod 360.d0
if v then print, 'Hour Angle = ', adstring(ha/15.d0), '  (hh:mm:ss)'
...
if refract_ then alt = $
      co_refract_mod(alt, altitude=altitude, _extra=_extra, /to_observed)

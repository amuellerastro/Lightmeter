;add computation of radius vector Earth-Sun (stolen from sun.pro)

PRO sunpos_mod, jd, ra, dec, longmed, delta_ES, oblt, RADIAN = radian
;+
; NAME:
;       SUNPOS
; PURPOSE:
...
; Compute radius vector Earth-Sun

  mna = 358.47583d0 + 35999.04975d0*t - 0.000150d0*t^2.d0 - 0.0000033d0*t^3.d0
  mna = mna mod 360d0

  c = (1.919460d0 - 0.004789d0*t - 0.000014d0*t^2.d0)*sin(mna*dtor) + (0.020094d0 - 0.000100d0*t)*sin(2.d0*mna*dtor) + 0.000293d0*sin(3.d0*mna*dtor)

  e = 0.01675104d0 - 0.0000418d0*t - 0.000000126d0*t^2.d0

  ta = (mna + c) mod 360d0

  delta_ES = 1.0000002d0*(1.d0 - e^2.d0)/(1.d0 + e*cos(ta*dtor))

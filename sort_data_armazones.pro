...
;search and selection of raw file
  file = file_search(pathraw+'*.*', count=nfiles)
  ...
  print, ''
  read, 'Which file, enter number?: ',usefile
...
;read in selected file, can be very large files, usage of readcol
  print, ''
  print, 'Reading Data...can take some time...'
  print, ''
  readcol, file, day, month, year, hh, mm, ss, t1, t2, dum1, counts, dum2, dum3, dum4, format='i,i,i,i,i,i,d,d,a,d,d,d,a', delimiter='.;:,', /silent
...
;extract each day with data - a day starts at 12:00am UT (thats why 'floor' is used)
  ...
  print, ''
  print, 'Wrting Data...can take some time...'
  print, ''
  for i=0L,n_elements(idx)-1 do begin

    idxval = where(jdround eq jdround[idx(i)])
...
;    printf, lun, '                JD    Temp.     Counts'
    for j=0L,n_elements(idxval)-1 do begin
      ...
      printf, lun, jd[idxval(j)], temp[idxval(j)], counts[idxval(j)], format='(f18.10,f9.1,i11.0)'
